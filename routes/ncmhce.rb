...
get '/ncmhce/scenarios/?' do
  ...
  @max_scenarios = User[session[:user]].max_scenarios
  @scenarios = Scenario.where(order: :id, active: true)
  @scenarios = @scenarios.where(workshop: false) unless user.workshop_scenarios == true

  @max_practice_scenarios = User[session[:user]].max_practice_scenarios

  @scenarios = @scenarios.where(practice: false) unless user.practice_exams == true
  @averages = Average.where(:user_id => session[:user], :scenario_id.not => nil)
  @remaining_scenarios = User[session[:user]].remaining_scenarios
  @uses = []
...
get '/ncmhce/scenarios/:id/?' do
  ...
  unless @scenario.sample? || @scenario.workshop? || @scenario.practice?
    if User[session[:user]].remaining_scenarios == 0
      unless Use.where(user_id: session[:user], scenario_id: params[:id]).count > 0
  ...
  if Average.where(scenario_id: params[:id], user_id: session[:user]).count > 0
    redirect "/ncmhce/scenarios/#{params[:id]}/score"
  end
  @scores = []
  Score.where(user_id: session[:user], scenario_id: params[:id]).each {|s| @scores << s.answer_id }
...
get '/ncmhce/scenarios/:id/score/?' do
  ...
  @scores = []
  scores = Score.where(user_id: session[:user], scenario_id: params[:id])
...
get '/ncmhce/scenarios/:id/restart/?' do
  ...
  Score.where(user_id: session[:user], scenario_id: params[:id]).destroy
  Average.where(scenario_id: params[:id], user_id: session[:user]).destroy
